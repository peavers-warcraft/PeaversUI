...
Eight addons, written from scratch, that replace six specific pieces of the World of Warcraft interface — and an installer that sets all of them up in one pass.

Every module in it that draws anything carries a performance budget in its own repository, re-measured by a benchmark on every push. Go over the budget and the build fails.

## Not another UI pack

The usual shape of a UI pack is one enormous addon plus a profile string: somebody else's framework, arranged to one author's taste, reskinning every frame the game has. This is not that, in two specific ways.

**All of it is ours.** There is no framework underneath and nothing here is a skin over another author's addon. Eight addons, each written for the single job it does, sharing one library between them rather than eight copies of the same one. Every module is a working standalone addon without the pack; the installer only ever writes ordinary settings into their own saved variables, which is why anything it sets can be changed afterwards in Edit Mode or in that module's own page.

**Performance is a number, not an adjective.** "Lightweight" in an addon README is a claim nobody can check. Every module here publishes a budget file, gets measured against it on every push by a harness whose source is public, and fails its build if it goes over. The numbers below are those measurements, not estimates.

**It replaces what is in the screenshot, and nothing else.** No action bars, no raid frames, no bags, no character panel. The full boundary is in the next section, and it is short on purpose.

## What it replaces, and what it does not

Six things:

- the player, target, target-of-target and focus frames, with their cast bars and auras
- the minimap, and the scattering of addon buttons around it
- the chat window
- tooltips
- the FPS and latency readout
- graphics presets, switched by what content you are in

That is the complete list. There is nothing behind it.

It does not touch:

- **Action bars.** Not moved, not skinned, not replaced.
- **Party and raid frames.** No file in the pack so much as names them.
- **Bags, the character panel, the spellbook, the map, the quest log** — or any other Blizzard frame.
- **Nameplates, boss timers, damage meters, WeakAuras.** Each of those deserves a specialist and there are good ones already; the [More stuff](#more-stuff) page names the ones the author runs.
- **Your keybinds and your Edit Mode layout.** The settings people have spent years arranging, and the ones a UI pack is most tempted to overwrite.

A pack that reskins your whole game cannot tell you what it costs, because "the whole game" is not a thing you can put a number on. Six replacements can be measured one at a time, which is the trade being made here.
...
### The whole pack, in one table

This is the interface the pack installs, and what it costs to draw:

| Module | Calls per frame | Budget | Idle calls/sec | Packaged |
|---|---:|---:|---:|---:|
| PeaversUnitFrames | 1.17 | 1.25 | 0 | 103.1 KB |
| PeaversMiniMap | 0 | 0 | 0 | 79 KB |
| PeaversChat | 0 | 0 | 0 | 192.3 KB |
| PeaversToolTip | 0 | 0 | 0 | 75.2 KB |
| PeaversSystemBars | 0 | 0 | 0 | 46.4 KB |
| PeaversUI (the installer) | 0 | 0 | 0 | 187.4 KB |

Those figures are each module's own published measurement, copied here so the pack can be judged as a whole; the authoritative table in every module's README is regenerated by the same harness on every push.

**One number in that table is not zero.** It is the cast bars in PeaversUnitFrames, which have to animate to be cast bars. Everything else is event driven: it costs nothing per frame, and nothing at all while you are standing still. Chat, tooltips and the system bars publish a calls-per-second figure for the work they do when something actually happens — 24, 43 and 32, each under its own budget — in their own READMEs, alongside the scenario each was measured in.

**Why you should believe it.** Each of those numbers is produced by the [Ultra Performance harness](https://github.com/peavers-code/peavers-warcraft-workflows/tree/master/perf-harness), which loads the addon's real shipped source into a Lua VM, stands in for the WoW API, counts every widget call it makes, and compares the result against a `perf/budget.json` committed next to the code. It runs in CI on every push. Exceeding a budget fails the build, so the numbers cannot quietly drift; the badge at the top of each README is generated by that run rather than typed in.

**What it does not prove.** The harness measures widget calls outside the game, not frames per second on your machine — a benchmark, not a promise about your hardware. It is a floor under the claim rather than the whole of it: it catches the per-frame loop nobody noticed, the ticker left running, the library quietly bundled in. That is a narrower claim than "makes your game faster", and it is the one that can actually be checked.
...
Negative claims rot quietly, so it is measured rather than asserted. Here the harness loads the real Config, Modules, Layouts and Installer, wires five stand-in module addons in front of them, and drives complete installs of every layout.
...
|---|---:|---:|:--:|
| Packaged size | 187.4 KB | 200 KB | pass |
...
<sub>4,798 lines of Lua · 187.4 KB packaged · no bundled libraries</sub>
...
<!-- peavers:features -->
- Not a reskin of somebody else's framework - eight addons written from scratch for the one job each of them does
- Replaces six things and nothing else: unit frames, minimap, chat, tooltips, the FPS and latency readout, and graphics presets
- Leaves your action bars, party and raid frames, bags, character panel and keybinds completely alone
- Every module that draws anything carries a performance budget in its own repository, measured on every push - go over it and the build fails
- One number in the whole interface is not zero: 1.17 widget calls per frame for the cast bars. Everything else is event driven
- The benchmark harness is public and the budget files are committed next to the code, so the numbers can be checked rather than taken on trust
...
**Everything it writes belongs to a Peavers module.** Nothing outside the six replacements listed at the top is edited — no keybinds, no action bars, no Edit Mode layout, no other author's saved variables — and every setting it does write can be changed afterwards from that module's own page or in Edit Mode.
